...
export async function buildPrintPack(
  ...
  // 1) Card mockup PNG — QR placed on a 3.5×2" white card at 300 DPI
  const cardMockupPath = await buildCardMockup(qrAbsPath, slug, ownerName)

  // 2) Sticker sheet PDF — 20× 1×1" QR stickers on A4
  const stickerPath = await buildStickerSheet(qrAbsPath, slug)

  // 3) README
  const readmePath = path.join(outDir, `${slug}-README.txt`)
  fs.writeFileSync(readmePath, buildReadme(ownerName, slug))

  // 4) Bundle all into ZIP
...
async function buildCardMockup(qrPath: string, slug: string, ownerName: string): Promise<string> {
  // 3.5×2" at 300 DPI = 1050×600px
  const cardW = 1050, cardH = 600
  const qrSize = 280   // ~0.93" at 300 DPI
...
async function buildStickerSheet(qrPath: string, slug: string): Promise<string> {
  ...
  // A4 at 72 DPI (PDFKit default): 595×842 pts
  // 1" = 72pt; sticker 1×1" = 72×72pt; margin 36pt; gap 10pt
  const stickerSize = 72  // 1 inch at 72 DPI
  const marginX = 36, marginY = 36, gap = 10
  const cols = Math.floor((595 - 2 * marginX + gap) / (stickerSize + gap))  // 6
  const rows = Math.floor((842 - 2 * marginY + gap) / (stickerSize + gap))  // 10

  const qrSmall = await sharp(qrPath)
...
function buildReadme(ownerName: string, slug: string): string {
  return `ChampQR Print Package
=======================
Card: ${ownerName}
Slug: ${slug}
...
qr-card-mockup.png — Preview of QR placed on a standard 3.5×2" business card.
qr-sticker-sheet.pdf — A4 sheet of 20× QR stickers at 1"×1". Print and stick on existing cards.
...
• For best AR tracking: print at 1.0" × 1.0" or larger.

HOW THE AR EXPERIENCE WORKS
-----------------------------
When someone scans this QR code with their phone camera, their browser opens and
the video plays directly on top of the QR code in augmented reality — no app required.
